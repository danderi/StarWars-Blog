...
export const SpeciesComponents = () => {
    ...
    return (
        <div style={{margin: "30px"}}>
            <h3 className="peopleComponents ms-3">Species {store.isLoadingSpecies === true ? <span className="fs-5"><LoadingComponent /></span> : <span></span>}</h3>
                        ...
                        {store.speciesDetails.map((species, index) => (
                            <div key={index} className="col-4" style={{ width: "300px", padding: "0 4px" }}>
                                <div className="card-main" style={{ width: "75%" }}>
                                    <img src={`https://starwars-visualguide.com/assets/img/species/${species.result.uid}.jpg`} className="card-img-top" alt="..." style={{ height: "10%" }} />
                                    <div className="card-body mb-3">
                                        <h5 className="card-title">{species.result.properties.name}</h5>
                                        <Link to={`/cardView/${species.result.uid}`} className="btn btn-outline-dark me-4">View Details</Link>
