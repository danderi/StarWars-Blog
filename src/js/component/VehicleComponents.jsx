...
export const VehicleComponents = () => {
    ...
    return (
        <div style={{margin: "30px"}}>
            <h3 className="peopleComponents ms-3">Vehicles {store.isLoadingVehicle === true ? <span className="fs-5"><LoadingComponent /></span> : <span></span>}</h3>
                        ...
                        {store.vehicleDetails.map((vehicle, index) => (
                            <div key={index} className="col-4" style={{ width: "300px", padding: "0 4px" }}>
                                <div className="card-main" style={{ width: "75%" }}>
                                    <img src={`https://starwars-visualguide.com/assets/img/vehicles/${vehicle.result.uid}.jpg`} className="card-img-top" alt="..." style={{ height: "10%" }} />
                                    <div className="card-body mb-3">
                                        <h5 className="card-title">{vehicle.result.properties.name}</h5>
                                        <Link to={`/cardView/${vehicle.result.uid}`} className="btn btn-outline-dark me-4">View Details</Link>
